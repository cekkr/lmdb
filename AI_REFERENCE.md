...
- The hot-path adapter now queues reducers via `PAIR_REDUCE_ASYNC` and polls `PAIR_REDUCE_FETCH`, so the TCP socket never sits idle for minutes while cheetah walks slow namespaces. Tweak `CHEETAH_REDUCE_ASYNC` (disable to fall back to synchronous reducers) plus `CHEETAH_REDUCE_POLL_INTERVAL_SECONDS` to adjust the keep-alive cadence; synchronous fallbacks still honor the idle-grace clamp.
