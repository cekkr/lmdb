...
- The hot-path adapter now retries stalled `PAIR_REDUCE` pages instead of disabling itself after the first timeout. Set `CHEETAH_REDUCER_RETRY_ATTEMPTS` (default 3) and `CHEETAH_REDUCER_RETRY_DELAY_SECONDS` (default 5s) to stretch or shorten the retry window when running over noisy links.
